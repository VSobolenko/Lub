					...
					case 1:{
						for (i=0; i<n-1; i++)
			 			for (j=i+1; j<n; j++)    
			   			...
			   			for(i=0;i<n;i++) {
							printf("\nPokupatel %d\n",i+1);
							inf(pokupateli[i].FIO, pokupateli[i].DomAdres, pokupateli[i].Nomer, pokupateli[i].kartochka);
			   			}
						break;
					}
					case 2:{
...
void inf(char *FIO, char *DomAdres, int *Nomer, int *kartochka){
	printf("\nF.I.O.: %s ",FIO);
	printf("\ndomashniy adres: %s ",DomAdres);
	printf("\nnomer telefona: %d ",Nomer);
	printf("\nnomer kartochki: %d ",kartochka);				
}
